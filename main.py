...
def plot(func, a, b, delta=0.5):  # draws the graph of func around the point (a,b)
    ...
    X, Y = np.meshgrid(np.linspace(a-delta, a+delta, 100),
                       np.linspace(b-delta, b+delta, 100))
    Z = func(X, Y)
    ...
    z_min = np.nanmin(Z)
    ax.contourf(X, Y, Z, 15, cmap='coolwarm',
                alpha=0.85, zdir='z', offset=z_min)
...
def grad(func, x, y, h=10**(-5)):  # returns approx grad for function at (x,y) as an array
    return (np.array([(func(x+h, y)-func(x, y))/h, (func(x, y+h)-func(x, y))/h]))


# plots the relative error between analytical and approx gradient of sin(x + y)
def gradplot(x, y, h=10**(-6), d=2*np.pi):
    ...
    plt.subplot(121)
    plt.title(
        r'Relative error: $\frac{\delta x_{Approx} - \delta x_{Analytical}}{\delta x_{Analytical}}$' + f'  for $h = {h}$')
    plt.pcolormesh(X, Y, (Z2[0] - Z1[0])/Z1[0], cmap='binary')
    plt.colorbar(location='bottom')
    plt.xticks(theta, ['-2π', '-3π/2', 'π', '-π/2',
               '0', 'π/2', 'π', '3π/2', '2π'])
    plt.yticks(theta, ['-2π', '-3π/2', 'π', '-π/2',
               '0', 'π/2', 'π', '3π/2', '2π'])
    plt.subplot(122)
    plt.title(
        r'Relative error: $\frac{\delta y_{Approx} - \delta y_{Analytical}}{\delta y_{Analytical}}$' + f'  for $h = {h}$')
...
for i in range(X.shape[0]):
    # iterate through the grid and do fszolve for z for each point
    for j in range(X.shape[1]):
        Z[i, j] = scipy.optimize.fsolve(lambda z: F(X[i, j], Y[i, j], z), 0)[0]

ax = plt.figure().add_subplot(projection='3d')
ax.plot_surface(X, Y, Z, edgecolor='royalblue', lw=0.5, rstride=8,
                cstride=8, alpha=0.3)  # plot the found surfacer


def f(a, b): return scipy.interpolate.RegularGridInterpolator((x, y), Z)(
    [[a, b]])[0]  # interpolate so that we have z(x,y) as a callable function


g = grad(f, 0, 0)
# we will use these every time in coefficients for P2, so calculating them here saves time
h = hessian(f, 0, 0)
...
ax.plot_surface(X, Y, Z1, edgecolor='red', lw=0.5,
                rstride=8, cstride=8, alpha=0.3)
plt.xlabel('X')
plt.ylabel('Y')
...
# heat map of the error in approximation
pcm = plt.pcolormesh(X, Y, abs(Z-Z1), cmap='inferno')
...
ax.plot_surface(X, Y, Z, edgecolor='royalblue', lw=0.5,
                rstride=8, cstride=8, alpha=0.3)
ax.contourf(X, Y, Z, 30, cmap='coolwarm', alpha=0.85,
            zdir='z', offset=np.nanmin(Z))
ax.set(xlim=(-5, 5), ylim=(-5, 5), zlim=(np.nanmin(Z), np.nanmax(Z)),
       xlabel='X', ylabel='Y', zlabel='Z')   # Add surface plot and contour to figure
